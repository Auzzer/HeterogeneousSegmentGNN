...
- **`pos`**  
  - **Description:** A 2D vector field holding the current positions of the cloth’s vertices (red dots).  
  - **Storage:** Dense field of size $NV = (N+1)^2$.

- **`initPos`**  
  - **Description:** Stores the initial (rest) positions of each vertex.  
  - **Usage:** Used to enforce fixed-point constraints.

- **`vel`**  
  - **Description:** The velocity of each particle (initialized to zero).  
  - **Storage:** Dense.

- **`mass`**  
  - **Description:** The mass value at each vertex.  
  - **Storage:** Dense.
...
- **Storage:**  
  Dense per edge; these edges are then assembled into sparse matrices along with the other springs.
...
- **`vel_1D`**, **`force_1D`**, **`b`**  
  ...
  - **Purpose:** They are used by `ti.linalg.SparseSolver` which expects flat vectors.
...
### 2. Forming the Global System and Simulation Update
...
where:
- $\Delta \mathbf{x}=\operatorname{pos}[i]-\operatorname{pos}[j]$
...
- $l=\|\Delta x\|$ (with $\Delta x=\operatorname{pos}[i]-\operatorname{pos}[j]$ ),
...
For a simple damping model $f_{\text {damp }}=-k_d\left(\operatorname{vel}_i-\operatorname{vel}_j\right)$ ，we have：
...
#### 2.3 Assembling Global Matrices
...
- $D$ is the global damping sparse matrix，assembled from the $J_v$（edge，vel）contributions：

$$
D \equiv \frac{\partial \mathbf{f}}{\partial \mathrm{vel}}
$$


- $K$ is the Sparse stiffness matrix，assembled from the $J_x$（edge，pos）contributions：

$$
K \equiv \frac{\partial \mathbf{f}}{\partial \mathrm{pos}}
$$


The assembly is done by iterating over each edge（spring）and adding its $2 \times 2$ block contributions to the appropriate locations in $K$ and $D$ ．For example，for an edge connecting vertices $i$ and $j$ ：\
The contribution to $K$ is：
...
A similar pattern is used for $D$ with $J_v$ ．

#### 2.4 Global System for Simulation Update
The final system solved is：$A \Delta v=b$
with:\
